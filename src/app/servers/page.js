...
export default function ServerPage() {
  ...
  const fetchGuilds = async (accessToken, retries = 3) => {
    try {
      ...
      if (!response.ok) {
        if (response.status === 429 && retries > 0) {
          ...
          setTimeout(() => fetchGuilds(accessToken, retries - 1), 1000); // Retry after 1 second
          return;
        }
                      ...
                      <img
                        src={
                          guild.icon
                            ...
                            : 'https://icons.veryicon.com/png/o/miscellaneous/open-ncloud/the-server-4.png' // Provide a path to a default icon
                        }
                        alt={`${guild.name} icon`}
